...
private:
    ...
    std::tuple<QueueTypes...> dataQueues;
    //tuple full of queues for the consumer to send empty objects to the producers
    std::tuple<QueueTypes...> returnQueues;
    
    //thread structures necessary for producer waiting
    typename TupleTypeGen<sizeof...(QueueTypes), boost::mutex>::TupleType producerWaitMutexes;
    typename TupleTypeGen<sizeof...(QueueTypes), boost::condition_variable>::TupleType producerWaitConditions;
    typename TupleTypeGen<sizeof...(QueueTypes), std::atomic_int>::TupleType producerWaitCounters;
